    ...
    if not creds or not creds.valid:
        ...
        if creds and creds.expired and creds.refresh_token:
            ...
            try:
                ...
                print("[DEBUG] trying Streamlit secrets")
                ...
                print("[DEBUG] created flow from Streamlit secrets")
            except (KeyError, FileNotFoundError, ImportError, Exception):
                # Fallback to local credentials.json file for development
                print("[DEBUG] fallback to credentials.json")
                
                if not os.path.exists(creds_path):
                    raise FileNotFoundError(
                        f"Google OAuth client secrets not found in st.secrets['google_credentials'] or at {creds_path}"
                    )
