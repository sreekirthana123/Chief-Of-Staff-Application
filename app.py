...
def triage_threads(threads: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def _make_flow(redirect_uri: str | None = None) -> GoogleFlow:
    """Build a Google OAuth 2.0 Flow from the local credentials.json file.

    If *redirect_uri* is given it overrides the default from the file.
    """
...
def _get_app_url() -> str:
    """Return the external URL of this Streamlit app (no trailing slash)."""
    try:
        # Streamlit Cloud sets this environment variable automatically
        base = os.environ.get("STREAMLIT_URL", "")
        if base:
            return base.rstrip("/")
        # Fallback: use the browser's host header
        from streamlit.web.server import Server
        server = Server.get_current()
        if server is not None:
            return f"http://localhost:{server.config.port}"
    except Exception:
        pass
    return "http://localhost:8501"


def _exchange_code_for_creds(auth_code: str) -> Credentials:
    """Exchange the OAuth authorization code (returned in the callback URL)
    for a refreshable Credentials object.

    The redirect URI used here *must* exactly match the one registered in
    the Google Cloud Console for this client.
    """
...
def _handle_oauth_callback() -> bool:
    ...
    try:
        creds = _exchange_code_for_creds(code)
...
def render_landing_page() -> None:
    ...
    if st.button(
        ...
        # Redirect the user's browser to Google's consent page
        st.markdown(
