...
def fetch_threads_via_engine() -> list[dict[str, Any]]:
    ...
    return fetch_gmail_threads(credentials_json=st.session_state.get("google_credentials"))
...
def render_landing_page() -> None:
    ...
    st.markdown(
        ...
        <a href="{auth_url}" target="_self" style="text-decoration: none;">
            <div style="background-color: #FF4B4B; color: white; padding: 10px 20px; border-radius: 5px; text-align: center; font-weight: bold;">
                🔑 Sign in with Google
            </div>
        </a>
        """,
