...
def apply_app_theme() -> None:
    ...
    button[kind="primary"]:disabled, button[kind="secondary"]:disabled {
        background-color: rgba(122, 94, 66, 0.15) !important;
        border: 1px solid rgba(122, 94, 66, 0.3) !important;
        color: rgba(58, 42, 30, 0.5) !important;
        border-radius: 10px !important;
...
def render_phase_title(text: str) -> None:
    st.markdown(f'<div class="phase-title">{text}</div>', unsafe_allow_html=True)
    ...
    .element-container > div,
      ul li,  /* Unordered lists */
       ol li {  /* Ordered lists */ 
        ...
        .stApp {
            background: 
                radial-gradient(circle at 15% 100%, rgba(75, 240, 252, 0.65) 0%, transparent 35%),
                radial-gradient(circle at 95% 100%, rgba(204, 255, 255, 0.45) 0%, transparent 40%),
                radial-gradient(circle at 100% 100%, rgba(154, 254, 255, 0.55) 0%, transparent 30%),
        ...
        .landing-wrapper {
            ...
            max-width: 100%;
            margin: 0 auto;
            padding: 100px 2%; /* Uses percentage to hug the edges */
        ...
        .hero-title {
            font-size: clamp(7.5rem, 5vw, 6.5rem); /* Huge main title */
            font-weight: 900;
