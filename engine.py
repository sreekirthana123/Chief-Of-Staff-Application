...
def _build_gmail_service(credentials_json: str | None = None):
    """
...
def fetch_threads_direct(
    ...
    """
    service = _build_gmail_service(credentials_json=credentials_json)
...
def send_reply(
    ...
    if service is None:
        raise Exception(
            "Gmail authentication required. Sign in with Google or switch to Sample threads."
        )
...
def fetch_threads(
    ...
    # ── Forced direct mode ──────────────────────────────────────────────
    if use_mcp is False:
        return fetch_threads_direct(max_results, credentials_json=credentials_json)

    # ── Auto-detect (use_mcp is None) ───────────────────────────────────
    creds_path = os.path.join(here, "credentials.json")
    if not os.path.exists(creds_path) and not credentials_json:
        return build_mcp_plan(max_results)
