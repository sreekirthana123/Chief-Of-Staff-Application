...
def _build_gmail_service():
    ...
    For Streamlit Cloud deployment, credentials are loaded from st.secrets instead of credentials.json.
    """
    ...
    try:
        ...
        if token_data:
            # Convert the token secret to a Credentials object
            creds = Credentials.from_authorized_user_info(dict(token_data))
    except (KeyError, AttributeError):
        pass
    
    # If no token in secrets, try local file
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            try:
                ...
                full_creds = {"installed": creds_dict}
            ...
            except (KeyError, FileNotFoundError, ImportError, Exception):
            ...
            if "could not locate runnable browser" in str(e) or "No web browser found" in str(e):
                ...
                **Gmail Authentication Required**
                ...
                To use Gmail features, you need to authenticate first:
                
                **Option 1: Upload Token (Recommended)**
                1. Run this app locally and complete Google login
                2. Copy the contents of the generated `token.json` file
                3. Add it to your Streamlit Cloud secrets as `google_token`
                
                **Option 2: Use Sample Data**
                - Switch to "Sample threads" in the sidebar to test the app
                """)
                # Return None to indicate Gmail is not available
                return None
            else:
                raise e
...
def fetch_threads_direct(max_results: int = DEFAULT_MAX_RESULTS) -> list[dict]:
    ...
    # If service is None, authentication failed (cloud environment)
    if service is None:
        return []
...
def send_reply(
    ...
    ----------
    thread_id : str
        The Gmail thread ID to reply within.
    to : str
        Recipient email address.
    subject : str
        Email subject (will be prefixed with "Re: " if not already present).
    body : str
        Plain-text email body.
    message_id : str | None
        The message ID being replied to (used to set In-Reply-To and References
    ...
    # Prepend "Re: " to subject if not already present (case-insensitive)
    ...
    # If message_id provided, set threading headers
    ...
    # If service is None, authentication failed (cloud environment)
    if service is None:
        raise Exception("Gmail authentication required. Please set up google_token in Streamlit secrets or switch to Sample threads.")
